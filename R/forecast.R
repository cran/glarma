...
forecast.glarma <- function(object, n.ahead = 1, newdata = 0,
  ...
  ## Tests for whether supplied data is of correct dimension
  ## Fix as response to email from Lucas Godoy <lucasdac.godoy@gmail.com>
  ## Tuesday, June 19, 2018 11:35 AM
  L <- n.ahead
  if (NROW(newdata) != L)
    stop("Number of values for each predictor must equal n.ahead")
  r <- object$r
  if (NCOL(newdata) != r)
    stop("newdata must have same number of columns as number of regressors")
  if ((!is.null(object$offset)) & (length(newoffset) != L))
    stop("n.ahead values of the offset must be supplied")
  if ((object$type == "Bin") & (length(newm) != L))
    stop("n.ahead values of the number of trials must be supplied")
