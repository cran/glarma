...
### Change to offset code, July 12, 2018, in response to email from
### polus@ibe.med.uni-muenchen.de <polus@ibe.med.uni-muenchen.de>
...
likeTests <- likTests <- function(object) {
  ...
  if (type == "NegBin"){
    ...
    if (is.null(offset)){
      ...
      (GLM <- glm.nb(y ~ -1 + X + offset(offset), init.theta = 1,
                     x = TRUE))
    }
  }
